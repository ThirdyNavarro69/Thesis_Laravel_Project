                            ...
                            <table class="table table-custom table-centered table-select table-hover table-bordered w-100 mb-0">
                                ...
                                <tbody>
                                    <?php $__empty_1 = true; $__currentLoopData = $bookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <tr data-booking-id="<?php echo e($booking->id); ?>">
                                            <td>
                                                <span class="fw-medium"><?php echo e($booking->booking_reference); ?></span>
                                                <small class="text-muted d-block"><?php echo e(ucfirst($booking->booking_type)); ?></small>
                                            </td>
                                            <td>
                                                <?php if($booking->booking_type === 'studio'): ?>
                                                    <?php echo e($booking->provider->studio_name ?? ($booking->provider->studio_name ?? 'Studio')); ?>

                                                <?php else: ?>
                                                    <?php echo e($booking->provider->brand_name ?? ($booking->provider->brand_name ?? 'Freelancer')); ?>

                                                <?php endif; ?>
                                            </td>
                                            <td><?php echo e($booking->category->category_name ?? 'N/A'); ?></td>
                                            <td>
                                                <?php if($booking->packages->count() > 0): ?>
                                                    <?php echo e($booking->packages->first()->package_name); ?>

                                                    <small class="text-muted d-block">₱<?php echo e(number_format($booking->packages->first()->package_price, 2)); ?></small>
                                                <?php else: ?>
                                                    N/A
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <div class="d-flex">
                                                    <div>
                                                        <p class="mb-1"><?php echo e(\Carbon\Carbon::parse($booking->event_date)->format('M d, Y')); ?></p>
                                                        <p class="mb-0 fs-xxs">
                                                            <span class="text-muted"><?php echo e($booking->start_time); ?> - <?php echo e($booking->end_time); ?></span>
                                                        </p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                ...
                                                ?>
                                                <span class="badge <?php echo e($statusBadge); ?> fs-8 px-2 w-100 text-uppercase"><?php echo e(str_replace('_', ' ', $booking->status)); ?></span>
                                            </td>
                                            <td>
                                                <?php
                                                    $paymentBadge = [
                                                ...
                                                ?>
                                                <span class="badge <?php echo e($paymentBadge); ?> fs-8 px-2 w-100 text-uppercase"><?php echo e(str_replace('_', ' ', $booking->payment_status)); ?></span>
                                                <small class="text-muted d-block mt-1 text-center">
                                                    ₱<?php echo e(number_format($totalPaid, 2)); ?> / ₱<?php echo e(number_format($booking->total_amount, 2)); ?>

                                                </small>
                                                <?php if($booking->payment_status === 'partially_paid'): ?>
                                                <div class="progress mt-1" style="height: 3px;">
                                                    <div class="progress-bar bg-info" role="progressbar" style="width: <?php echo e($paymentProgress); ?>%;" 
                                                        aria-valuenow="<?php echo e($paymentProgress); ?>" aria-valuemin="0" aria-valuemax="100"></div>
                                                </div>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <span class="fw-semibold">₱<?php echo e(number_format($booking->total_amount, 2)); ?></span>
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center gap-1">
                                                    ...
                                                    <?php if(in_array($booking->status, ['confirmed', 'in_progress']) && in_array($booking->payment_status, ['pending', 'partially_paid'])): ?>
                                                        <?php
                                                            $totalPaid = $booking->payments->where('status', 'succeeded')->sum('amount');
                                                        ...
                                                        <button class="btn btn-sm pay-balance-btn" 
                                                                data-booking-id="<?php echo e($booking->id); ?>"
                                                                data-booking-reference="<?php echo e($booking->booking_reference); ?>"
                                        ...
                                        <tr>
                                            <td colspan="9" class="text-center py-4">
                                                <div class="text-muted">
                                                    ...
                                                    <div class="mt-3">
                                                        <a href="<?php echo e(route('client.dashboard')); ?>" class="btn btn-primary btn-sm">
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadPaymentDetailsForModal(bookingId, bookingRef) {
                $.ajax({
                    url: '<?php echo e(route("client.booking.payment.details", ":id")); ?>'.replace(':id', bookingId),
                    type: 'GET',
                    beforeSend: function() {
            ...
            function initializeBalancePayment(bookingId, amount) {
                $.ajax({
                    url: '<?php echo e(route("client.booking.balance.payment", ":id")); ?>'.replace(':id', bookingId),
                    type: 'POST',
                    data: {
                        amount: amount,
            ...
            function proceedWithBalancePayment(bookingId, paymentId, bookingRef, amount) {
                $.ajax({
                    url: '<?php echo e(route("client.payments.initialize")); ?>',
                    type: 'POST',
                    data: {
            ...
            function loadBookingDetails(bookingId) {
                $.ajax({
                    url: '<?php echo e(route("client.booking.details", ":id")); ?>'.replace(':id', bookingId),
                    type: 'GET',
                    beforeSend: function() {
                                    ...
                                    <div class="d-flex align-items-center p-3 border rounded">
                                        ${provider ? `
                                            <div class="avatar-xl me-3">
                                                <img src="${provider && (providerType === 'studio' ? (provider.studio_logo ? '<?php echo e(asset("storage/")); ?>/' + provider.studio_logo : '<?php echo e(asset("assets/images/sellers/7.png")); ?>') : (provider.brand_logo ? '<?php echo e(asset("storage/")); ?>/' + provider.brand_logo : '<?php echo e(asset("assets/images/sellers/3.png")); ?>')) || '<?php echo e(asset("assets/images/sellers/7.png")); ?>'}" 
            ...
            function cancelBooking(bookingId) {
                $.ajax({
                    url: '<?php echo e(route("client.booking.cancel", ":id")); ?>'.replace(':id', bookingId),
                    type: 'POST',
                    data: {
                        _token: '<?php echo e(csrf_token()); ?>'
                    },
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.client.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\proj\resources\views/client/view-my-bookings.blade.php ENDPATH**/ ?>