...
class BookingDetailsController extends Controller
{
    ...
    /**
     * Display booking form for studio or freelancer.
     */
    public function book($type, $id)
    {
        if ($type === 'studio') {
            // Fetch studio details for the booking form
            $studio = StudiosModel::whereIn('status', ['approved', 'active', 'verified'])
                ->with(['location', 'category', 'packages'])
                ->findOrFail($id);

            return view('client.booking-forms', compact('studio', 'type'));
        }
        
        // For freelancer
        if ($type === 'freelancer') {
            // Fetch freelancer details for the booking form
            $freelancer = ProfileModel::with(['user', 'location', 'categories'])
                ->whereHas('user', function($query) {
                    $query->where('status', 'active');
                })
                ->findOrFail($id);

            return view('client.booking-forms', compact('freelancer', 'type'));
        }
